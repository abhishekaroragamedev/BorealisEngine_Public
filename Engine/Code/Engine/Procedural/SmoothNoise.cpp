...
#include "Engine/Procedural/RawNoise.hpp"
...
float Compute3dPerlinNoise( float posX, float posY, float posZ, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed )
{
	...
	{
		Vector3( +ROOT_THREE_BY_THREE, +ROOT_THREE_BY_THREE, +ROOT_THREE_BY_THREE ), // Normalized unit 3D vectors
		Vector3( -ROOT_THREE_BY_THREE, +ROOT_THREE_BY_THREE, +ROOT_THREE_BY_THREE ), //  pointing toward cube
		Vector3( +ROOT_THREE_BY_THREE, -ROOT_THREE_BY_THREE, +ROOT_THREE_BY_THREE ), //  corners, so components
		Vector3( -ROOT_THREE_BY_THREE, -ROOT_THREE_BY_THREE, +ROOT_THREE_BY_THREE ), //  are all sqrt(3)/3, i.e.
		Vector3( +ROOT_THREE_BY_THREE, +ROOT_THREE_BY_THREE, -ROOT_THREE_BY_THREE ), // 0.5773502691896257645091f.
		Vector3( -ROOT_THREE_BY_THREE, +ROOT_THREE_BY_THREE, -ROOT_THREE_BY_THREE ), // These are slightly better
		Vector3( +ROOT_THREE_BY_THREE, -ROOT_THREE_BY_THREE, -ROOT_THREE_BY_THREE ), // than axes (1,0,0) and much
		Vector3( -ROOT_THREE_BY_THREE, -ROOT_THREE_BY_THREE, -ROOT_THREE_BY_THREE )  // faster than edges (1,1,0).
	};
...
float Compute3dPerlinNoiseWrapped( float posX, float posY, float posZ, float scaleX/* =1.f */, float scaleY/* =1.f */, float scaleZ/* =1.f */, unsigned int numOctaves/* =1 */, float octavePersistence/* =0.5f */, float octaveScale/* =2.f */, bool renormalize/* =true */, unsigned int seed/* =0 */ )
{
	const float OCTAVE_OFFSET = 0.636764989593174f; // Translation/bias to add to each octave
	...
	float totalAmplitude = 0.f;
	float currentAmplitude = 1.f;
	Vector3 currentPos( posX / scaleX, posY / scaleY, posZ / scaleZ );
	...
	for( unsigned int octaveNum = 0; octaveNum < numOctaves; ++ octaveNum )
	{
		...
		currentPos.y += OCTAVE_OFFSET; // Add "irrational" offset to de-align octave grids
		currentPos.z += OCTAVE_OFFSET; // Add "irrational" offset to de-align octave grids
		if ( IsFloatGreaterThanOrEqualTo( currentPos.x, currentScale.x ) )	{	currentPos.x -= currentScale.x;	}
		if ( IsFloatGreaterThanOrEqualTo( currentPos.y, currentScale.y ) )	{	currentPos.y -= currentScale.y;	}
		if ( IsFloatGreaterThanOrEqualTo( currentPos.z, currentScale.z ) )	{	currentPos.z -= currentScale.z;	}
