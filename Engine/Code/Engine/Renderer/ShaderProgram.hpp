...
public:
	ShaderProgram();
	ShaderProgram( const tinyxml2::XMLElement& shaderProgramElement, bool isCompute = false );
	ShaderProgram( const ShaderProgram& copy );
	~ShaderProgram();

	void LoadFromXML( const tinyxml2::XMLElement& shaderProgramElement, bool isCompute = false );
	...
	bool LoadComputeFromFile( const char* root, const std::string& semiColonDelimitedDefines = "" );
...
private:
	...
	static GLuint LinkComputeShaderAndCreateProgram( GLint computeShader );
