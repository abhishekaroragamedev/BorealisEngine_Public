...
void SpriteAnimationDefinition::PopulateSpriteSheetInfo( const tinyxml2::XMLElement& spriteAnimElement, Renderer& renderer )
{
	...
	{
		Texture* spriteSheetTexture = renderer.CreateOrGetTexture( std::string( SPRITE_SHEET_DIRECTORY_PATH ) + overriddenSpriteSheetName, 1U, false ); // Sprite system doesn't support async load as it needs dimensions defined beforehand
	...
	else	// Use the default spritesheet referenced by the <SpriteAnimSet> tag
	{
		...
		Texture* spriteSheetTexture = renderer.CreateOrGetTexture( std::string( SPRITE_SHEET_DIRECTORY_PATH ) + defaultSpriteSheetName, 1U, false ); // Sprite system doesn't support async load as it needs dimensions defined beforehand
