...
void SpriteAnimationSet::SetCurrentSpriteAnimation( const std::string& nextSpriteAnimationName )
{
	...
	if ( m_currentSpriteAnimation->GetDefinition()->GetName() != nextSpriteAnimationName )
	{
...
Sprite* SpriteAnimationSet::GetCurrentSpriteFromSpriteAnimation()
{
	if ( !m_currentSpriteAnimation->GetDefinition()->GetSpriteSheet()->UsesSprites() )
	{
		return nullptr;
	}

	return m_currentSpriteAnimation->GetCurrentSprite();
}
