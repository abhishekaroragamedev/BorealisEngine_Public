...
SpriteSheet::SpriteSheet( Texture& texture, int tilesWide /* = 0 */, int tilesHigh /* = 0 */, bool usesSprites /* = false */, bool flipY /* = false */ )
...
int SpriteSheet::AddSprite( Sprite* sprite )
{
	if ( m_flipY )
	{
		sprite->m_UVPixels.mins.y = sprite->m_texture->GetDimensionsF().y - sprite->m_UVPixels.mins.y;
		sprite->m_UVPixels.maxs.y = sprite->m_texture->GetDimensionsF().y - sprite->m_UVPixels.maxs.y;
	}
