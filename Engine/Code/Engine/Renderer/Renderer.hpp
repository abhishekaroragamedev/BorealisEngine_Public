...
constexpr char COMPUTE_SHADERS_RELATIVE_PATH[] = "Compute/";
...
public:
	...
	Texture* CreateOrGetTexture( const std::string& textureFilePath, unsigned int mipLevels = 1U, bool loadAsync = true, TextureFormat format = TEXTURE_FORMAT_RGBA8 );
	Texture3D* CreateOrGetTexture3D( unsigned int numFiles, const std::string* fileNames, unsigned int mipLevels = 1U, TextureFormat format = TEXTURE_FORMAT_RGBA8 );	// Texture will be indexed under first filename
	...
	ShaderProgram* CreateOrGetComputeShaderProgram( const std::string& shaderName, const std::string& semiColonDelimitedDefines = "" );
	ShaderProgram* CreateOrGetComputeShaderProgram( const char* shaderName, const char* semiColonDelimitedDefines = "" );
	...
	Texture* CreateFromImage( const Image& image, unsigned int mipLevels = 1U, TextureFormat format = TEXTURE_FORMAT_RGBA8 ) const;
	...
	int GetGLTextureAccessType( const TextureAccessType accessType ) const;
	...
	void BindTexture3DAndSampler( unsigned int slotIndex, const Texture3D& texture, const Sampler& sampler ) const;
	void BindTexture3DAsImage( Texture3D* texture, TextureAccessType access );
	...
	void DrawFullScreenImmediate( Material& material, const Rgba& color = Rgba::WHITE, unsigned int passIndex = 0U, bool useViewport = true );
	...
	void DispatchCompute( ShaderProgram* computeProgram, int numWorkGroupsX, int numWorkGroupsY, int numWorkGroupsZ );
...
private:
	...
	std::map< std::string, Texture3D* > m_loaded3DTexturesByName;
