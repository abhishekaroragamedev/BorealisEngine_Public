...
ShaderProgram::ShaderProgram( const tinyxml2::XMLElement& shaderProgramElement, bool isCompute /* = false */ )
{
	LoadFromXML( shaderProgramElement, isCompute );
}
...
void ShaderProgram::LoadFromXML( const tinyxml2::XMLElement& shaderProgramElement, bool isCompute /* = false */ )
{
	if ( m_info != nullptr )
	{
		delete m_info;
	...
	if ( fileName != "" )
	{
		if ( isCompute )
		{
			LoadComputeFromFile( fileName.c_str(), defines );
		}
		else
		{
...
bool ShaderProgram::LoadComputeFromFile( const char* root, const std::string& semiColonDelimitedDefines /* = "" */ )
{
	std::string filename = std::string( root ) + ".compute";

	GLuint computeShader = LoadShaderFromFile( filename.c_str(), GL_COMPUTE_SHADER, semiColonDelimitedDefines );
	if ( computeShader == NULL )
	{
		return false;
	}

	m_programHandle = LinkComputeShaderAndCreateProgram( computeShader );
...
/* static */
GLuint ShaderProgram::LinkComputeShaderAndCreateProgram( GLint computeShader )
{
	GLuint programID = glCreateProgram();
	GUARANTEE_RECOVERABLE( programID != 0, "Cannot find shader program!" );
