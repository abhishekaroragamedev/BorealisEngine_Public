...
//
Texture::Texture( const std::string& filePath, unsigned int mipLevels /* = 1U */, bool loadAsync /* = true */, TextureFormat format /* = TEXTURE_FORMAT_RGBA8 */ )
	: m_textureID( 0 )
	, m_dimensions( 0, 0 )
	, m_format( format )
{
	CreateFromFile( filePath, mipLevels, loadAsync, format );
}
...
void Texture::CreateFromImage( const Image& image, unsigned int mipLevels /* = 1U */, TextureFormat format /* = TextureFormat::TEXTURE_FORMAT_RGBA8 */ )
{
	...
	GLenum internalFormat = ( format == TEXTURE_FORMAT_RGBA32 )? GL_RGBA32F : GL_RGBA8; // the format we want the texture to be on the card; allows us to translate into a different texture format as we upload to OpenGL
	...
	glTexSubImage2D( GL_TEXTURE_2D,
		...
		GL_FLOAT,     // how are those channels stored
...
void Texture::CreateFromFile( const std::string& imageFilePath, unsigned int mipLevels /* = 1U */, bool loadAsync /* = true */, TextureFormat format /* = TextureFormat::TEXTURE_FORMAT_RGBA8 */ )
{
	if ( loadAsync )
	{
	...
	else
	{
		...
		CreateFromImage( imageFromFile, mipLevels, format );
	}
}
...
void TextureFormatToGLFormats( GLenum* internalFormat, GLenum* channels, GLenum* pixelLayout, TextureFormat format )
{
	switch ( format )
	{
		case TextureFormat::TEXTURE_FORMAT_RGBA8	:
		{
			...
			*channels = GL_RGBA;
			*pixelLayout = GL_FLOAT;
			break;
		}
		case TextureFormat::TEXTURE_FORMAT_RGBA32	:
		{
