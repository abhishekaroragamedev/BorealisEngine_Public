	...
	for ( const tinyxml2::XMLElement* childElement = shaderPassElement.FirstChildElement(); childElement != nullptr; childElement = childElement->NextSiblingElement() )
	{
		...
		else if ( std::string( childElement->Name() ) == "Blend" )
		{
			for ( const tinyxml2::XMLElement* grandChildElement = childElement->FirstChildElement(); grandChildElement != nullptr; grandChildElement = grandChildElement->NextSiblingElement() )
			{
				...
				{
					m_state.m_colorBlendOperation = GetBlendOperationFromName( ParseXmlAttribute( *grandChildElement, "operation", "" ) );
					m_state.m_colorSourceFactor = GetBlendFactorFromName( ParseXmlAttribute( *grandChildElement, "source", "" ) );
					m_state.m_colorDestinationFactor = GetBlendFactorFromName( ParseXmlAttribute( *grandChildElement, "destination", "" ) );
				}
				else if ( std::string( grandChildElement->Name() ) == "Alpha" )
				{
					m_state.m_alphaBlendOperation = GetBlendOperationFromName( ParseXmlAttribute( *grandChildElement, "operation", "" ) );
					m_state.m_alphaSourceFactor = GetBlendFactorFromName( ParseXmlAttribute( *grandChildElement, "source", "" ) );
					m_state.m_alphaDestinationFactor = GetBlendFactorFromName( ParseXmlAttribute( *grandChildElement, "destination", "" ) );
				}
			}
		}
...
{
	m_state.m_colorBlendOperation = operation;
	m_state.m_colorSourceFactor = sourceFactor;
	m_state.m_colorDestinationFactor = destinationFactor;
}
...
{
	m_state.m_alphaBlendOperation = operation;
	m_state.m_alphaSourceFactor = sourceFactor;
	m_state.m_alphaDestinationFactor = destinationFactor;
}

void ShaderPass::DisableColorBlending()
{
	m_state.m_colorBlendOperation = RendererBlendOperation::OPERATION_ADD;
	m_state.m_colorSourceFactor = RendererBlendFactor::FACTOR_ONE;
	m_state.m_colorDestinationFactor = RendererBlendFactor::FACTOR_ZERO;
}

void ShaderPass::DisableAlphaBlending()
{
	m_state.m_alphaBlendOperation = RendererBlendOperation::OPERATION_ADD;
	m_state.m_alphaSourceFactor = RendererBlendFactor::FACTOR_ONE;
	m_state.m_alphaDestinationFactor = RendererBlendFactor::FACTOR_ONE;
}
