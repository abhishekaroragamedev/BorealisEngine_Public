...
Renderer::~Renderer()
{
	...
	for ( std::map< std::string, Texture3D* >::iterator mapIterator = m_loaded3DTexturesByName.begin(); mapIterator != m_loaded3DTexturesByName.end(); mapIterator++ )
	{
		if ( mapIterator->second != nullptr )		// Deallocate memory for Texture objects (in main memory)
		{
			delete mapIterator->second;
...
bool Renderer::RenderStartup()
{
	...
	HGLRC realRenderingContext = CreateRealRenderContext( 4, 3 );
...
Texture* Renderer::CreateOrGetTexture( const std::string& textureFilePath, unsigned int mipLevels /* = 1U */, bool loadAsync /* = true */, TextureFormat format /* = TEXTURE_FORMAT_RGBA8 */ )
{
	if ( m_loadedTexturesByName.find( textureFilePath ) == m_loadedTexturesByName.end() )
	{
		m_loadedTexturesByName[ textureFilePath ] = new Texture( textureFilePath, mipLevels, loadAsync, format );
	}

	return m_loadedTexturesByName[ textureFilePath ];
}

Texture3D* Renderer::CreateOrGetTexture3D( unsigned int numFiles, const std::string* fileNames, unsigned int mipLevels /* = 1U */, TextureFormat format /* = TEXTURE_FORMAT_RGBA8 */ )
{
	if ( m_loaded3DTexturesByName.find( fileNames[ 0 ] ) == m_loaded3DTexturesByName.end() )
	{
		Texture3D* newTexture = new Texture3D();
		newTexture->CreateFromFiles( numFiles, fileNames, mipLevels, format );
		m_loaded3DTexturesByName[ fileNames[ 0 ] ] = newTexture;
	}

	return m_loaded3DTexturesByName[ fileNames[ 0 ] ];
}
...
Texture* Renderer::CreateFromImage( const Image& image, unsigned int mipLevels /* = 1U */, TextureFormat format /* = TEXTURE_FORMAT_RGBA8 */ ) const
{
	Texture* texture = new Texture();
	texture->CreateFromImage( image, mipLevels, format );
	return texture;
}
...
ShaderProgram* Renderer::CreateOrGetComputeShaderProgram( const std::string& shaderName, const std::string& semiColonDelimitedDefines /* = "" */ )
{
	if ( m_loadedShaderProgramsByName.find( shaderName ) == m_loadedShaderProgramsByName.end() )
	{
		ShaderProgram* newShaderProgram = new ShaderProgram();
		...
		// Try to load the provided file(s), and if it fails, load the invalid shader into the same object
		if ( !newShaderProgram->LoadComputeFromFile( ( std::string( SHADERS_RELATIVE_PATH ) + std::string( COMPUTE_SHADERS_RELATIVE_PATH ) + shaderName ).c_str(), semiColonDelimitedDefines ) )
		{
			delete newShaderProgram;
			LogErrorf( "Renderer::CreateOrGetComputeShaderProgram: Failed to create compute shader program for shader name %s.", shaderName.c_str() );
			return nullptr;
		}
...
ShaderProgram* Renderer::CreateOrGetComputeShaderProgram( const char* shaderName, const char* semiColonDelimitedDefines /* = "" */ )
{
	return CreateOrGetComputeShaderProgram( std::string( shaderName ), std::string( semiColonDelimitedDefines ) );
}
...
void Renderer::BindRenderState( const RenderState& renderState ) const
{
	...
	glBlendEquationSeparate(
		GetGLBlendOperation( renderState.m_blendOptions.m_colorBlendOperation ),
		GetGLBlendOperation( renderState.m_blendOptions.m_alphaBlendOperation )
	);
	glBlendFuncSeparate(
		GetGLBlendFactor( renderState.m_blendOptions.m_colorSourceFactor ),
		GetGLBlendFactor( renderState.m_blendOptions.m_colorDestinationFactor ),
		GetGLBlendFactor( renderState.m_blendOptions.m_alphaSourceFactor ),
		GetGLBlendFactor( renderState.m_blendOptions.m_alphaDestinationFactor )
	);
	// Blend overrides for individual Render targets
	std::vector< RenderBlendOptions > targetBlendOptions = renderState.m_targetBlendOptions;
	for ( size_t targetIndex = 0U; targetIndex < targetBlendOptions.size(); targetIndex++ )
	{
		glBlendEquationSeparatei(
			( GLuint )targetIndex,
			GetGLBlendOperation( targetBlendOptions[ targetIndex ].m_colorBlendOperation ),
			GetGLBlendOperation( targetBlendOptions[ targetIndex ].m_alphaBlendOperation )
		);
		glBlendFuncSeparatei(
			( GLuint )targetIndex,
			GetGLBlendFactor( targetBlendOptions[ targetIndex ].m_colorSourceFactor ),
			GetGLBlendFactor( targetBlendOptions[ targetIndex ].m_colorDestinationFactor ),
			GetGLBlendFactor( targetBlendOptions[ targetIndex ].m_alphaSourceFactor ),
			GetGLBlendFactor( targetBlendOptions[ targetIndex ].m_alphaDestinationFactor )
		);
	}
...
Vector4 Renderer::GetAmbientLight() const
{
	return m_activeLightsStruct.m_ambientLightColorAndIntensity;
}
...
int Renderer::GetGLTextureAccessType( const TextureAccessType accessType ) const
{
	switch ( accessType )
	{
		case TextureAccessType::TEXTURE_ACCESS_READ_ONLY	:	return GL_READ_ONLY;
		case TextureAccessType::TEXTURE_ACCESS_WRITE_ONLY	:	return GL_WRITE_ONLY;
		case TextureAccessType::TEXTURE_ACCESS_READ_WRITE	:	return GL_READ_WRITE;
...
void Renderer::BindTexture3DAndSampler( unsigned int slotIndex, const Texture3D& texture, const Sampler& sampler ) const
{
	glActiveTexture( GL_TEXTURE0 + slotIndex );
...
void Renderer::BindTexture3DAsImage( Texture3D* texture, TextureAccessType access )
{
	glBindImageTexture(
		0,
		texture->m_textureID,
...
void Renderer::DrawFullScreenImmediate( Material& material, const Rgba& color /* = Rgba::WHITE */, unsigned int passIndex /* = 0U */, bool useViewport /* = true */ )
{
	BindMaterial( material, passIndex );

	AABB2 currentViewport = AABB2::ZERO_TO_ONE;
	if ( useViewport )
	{
		currentViewport = m_currentCamera->GetViewport();
	}
...
void Renderer::DrawSprite( Sprite* sprite, const Vector3& position, const Vector3& rightVector, float spriteScale, const Rgba& tint, Vector2 uvFlip /* = Vector2::ONE */ )
{
	...
	Vector2 spriteSize = sprite->GetRenderDimensions();
	Vector2 effectiveDimensions = spriteScale * spriteSize;
...
void Renderer::DispatchCompute( ShaderProgram* computeProgram, int numWorkGroupsX, int numWorkGroupsY, int numWorkGroupsZ )
{
	UseShaderProgram( computeProgram );
...
void Renderer::SaveScreenshot() const
{
	...
	GLfloat* imageData = new float[ 4 * imageDimensions.x * imageDimensions.y ];
	glReadPixels( 0, 0, imageDimensions.x, imageDimensions.y, GL_RGBA, GL_FLOAT, imageData );

	GLubyte* imageDataUint = new unsigned char[ 4 * imageDimensions.x * imageDimensions.y ];
	for ( int texelIndex = 0; texelIndex < ( 4 * imageDimensions.x * imageDimensions.y ); texelIndex++ )
	{
		imageDataUint[ texelIndex ]	= static_cast< unsigned int >( imageData[ texelIndex ] * RGBA_MAX_F );
	}
	delete[] imageData;

	SaveScreenshotThreadArgs* args = new SaveScreenshotThreadArgs();
	args->m_imageData = imageDataUint;
