...
constexpr float ROOT_THREE_BY_THREE = 0.5773502691896257645091f;
...
float Floor( float a );
float Ceil( float a );
void FractionToBits( float fract, int numBits, char* out_bits );	// Converts the fractional part of a number to bits
void ReverseBits( char* bits, int numBits, char* out_reverse );
float BitsToFraction( char* bits, int numBits );	// Converts the bits of the int to a fraction
