...
void IntVector2::SetFromText( const std::string& text )
{
	TokenizedString tokenizedString = TokenizedString( text, "," );
	std::vector< std::string > tokens = tokenizedString.GetTokens();
...
void IntVector2::operator=( const IntVector2& assignedVector )
{
	x = assignedVector.x;
	y = assignedVector.y;
}
...
IntVector2 IntVector2::GetAsSigns( const IntVector2& intVector )
{
	return IntVector2( Sign( intVector.x ), Sign( intVector.y ) );
}
