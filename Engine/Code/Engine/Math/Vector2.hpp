...
public:
	...
	void TurnTowardVector2( const Vector2& target, float maxTurnDegrees );
...
public:
	...
	Vector2 GetComponentInDirection( const Vector2& direction ) const;
...
public:																// Operators
	...
	const Vector2 operator*( const Vector2& nonUniformScale ) const;	// ( float * float ) for both elements
	const Vector2 operator/( const Vector2& nonUniformDivisor ) const;	// ( float / float ) for both elements
	...
	void operator*=( const Vector2& nonUniformScale );				// vec2 *= vec2
	void operator/=( const Vector2& nonUniformDivisor );			// vec2 /= vec2
...
public:
	...
	static bool IsValidString( const std::string& vec2AsString );
