...
//-----------------------------------------------------------------------------------------------
static void LogOutputToIDE( const Log& log, void* )
{
	std::string logMessage = log.GetLogString();
...
//-----------------------------------------------------------------------------------------------
void DebugRegisterLogHook()
{
	LogHookRegister( LogOutputToIDE );
}
...
//	std::string appName = theApplication ? theApplication->GetApplicationName() : "Unnamed Application";
	...
	LogErrorf( "\n==============================================================================\n" );
	LogErrorf( "RUN-TIME FATAL ERROR on line %i of %s, in %s()\n", lineNum, fileName, functionName );
...
//	std::string appName = theApplication ? theApplication->GetApplicationName() : "Unnamed Application";
	...
	LogWarningf( "\n------------------------------------------------------------------------------\n" );
	LogWarningf( "RUN-TIME RECOVERABLE WARNING on line %i of %s, in %s()\n", lineNum, fileName, functionName );
	LogWarningf( "%s(%d): %s\n", filePath, lineNum, errorMessage.c_str() ); // Use this specific format so Visual Studio users can double-click to jump to file-and-line of error
	LogWarningf( "------------------------------------------------------------------------------\n\n" );
