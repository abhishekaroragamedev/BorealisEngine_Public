...
void Clock::StepClock( uint64_t timeElapsed )
{
	...
	m_totalTime.seconds = PerformanceCounterToSeconds( m_totalTime.hpc );
	m_totalTime.milliSeconds = static_cast<unsigned int>( m_totalTime.seconds * 1000.0 );
...
void Clock::SetParent( Clock* newParent )
{
	if ( m_parentClock )
	{
		std::vector< Clock* >::iterator foundClock = std::find( m_parentClock->m_childClocks.begin(), m_parentClock->m_childClocks.end(), this );
		if ( foundClock != m_parentClock->m_childClocks.end() )
		{
			m_parentClock->m_childClocks.erase( foundClock );
		}
	}
