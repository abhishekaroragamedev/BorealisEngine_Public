...
{
	UNUSED( message );
	//ConsolePrintf( "Received heartbeat from %s.", connection.m_address.ToString().c_str() );
	return true;
}
...
void NetSession::RegisterMessage( const std::string& name, message_cb callback, unsigned int options /* = 0U */ )
{
	NetMessageDefinition newDefinition = NetMessageDefinition( name, callback, options );
...
bool NetSession::RegisterMessage( int messageIndex, const std::string& name, message_cb callback, unsigned int options /* = 0U */ )
{
	...
	NetMessageDefinition newDefinition = NetMessageDefinition( name, callback, options, messageIndex );
...
NetConnection* NetSession::AddConnection( uint8_t index, const NetAddressIPv4& addr )
{
	if ( addr == m_socket->m_netAddress )
	{
		if ( m_myConnectionIndex != INVALID_CONNECTION_INDEX )
		{
			ConsolePrintf( Rgba::YELLOW, "NetSession::AddConnection(): Trying to add local connection a second time. Returning original local connection." );
			return m_connections[ m_myConnectionIndex ];
		}

		ConsolePrintf( Rgba::GREEN, "NetSession::AddConnection(): Added local connection at index %u.", index );
		m_myConnectionIndex = static_cast< uint8_t >( index );
	}
...
void NetSession::SetSendRate( float sendRateHz )
{
	...
	{
		NetConnection* connection = m_connections[ connIndex ];
		if ( connection != nullptr )
		{
...
void NetSession::SetHeartRate( float heartRateHz )
{
	...
	for ( uint8_t connIndex = 0U; connIndex < SESSION_MAX_CONNECTIONS; connIndex++ )
	{
		...
		{
			m_connections[ connIndex ]->SetHeartRate( heartRateHz );
		}
	}
}
...
NetConnection* NetSession::GetConnectionWithAddress( const NetAddressIPv4& addr ) const
{
	...
	{
		if ( m_connections[ i ] != nullptr && m_connections[ i ]->m_address == addr )
		{
...
void NetSession::ProcessIncoming()
{
	...
	if ( received > 0U )
	{
		...
		else
		{
			...
			// LogPrintf( "NetSession::ProcessIncoming(): Received packet %u.", *ackPtr );
		}
	}
