...
void main( void )
{
	float timeRemainingFraction = PASSTIMEREMAINING/PASSTOTALTIME;

	vec4 diffuse = texture( gTexDiffuse, PASSUV );
	vec4 edgeColor = vec4( vec3( GetEdgeColor() ), 1 );
	vec4 finalColor = diffuse * PASSCOLOR * edgeColor;
	finalColor.a = finalColor.a * timeRemainingFraction;
	outColor = finalColor;
}
