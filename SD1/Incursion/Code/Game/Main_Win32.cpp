...
void CreateOpenGLWindow( HINSTANCE applicationInstanceHandle, const float clientAspect, LPCCH APP_NAME, WNDPROC WindowsMessageHandlingProcedure )
{
	...
	HMONITOR targetMonitor = MonitorFromWindow( *g_windowHandle, MONITOR_DEFAULTTONEAREST );		// Now, make the window go fullscreen
	MONITORINFO monitorInfo;
	monitorInfo.cbSize = sizeof( MONITORINFO );
	GetMonitorInfo( targetMonitor, &monitorInfo );
	RECT destinationPosition = monitorInfo.rcMonitor;

	DWORD style = WS_POPUP;
	DWORD exStyle = WS_EX_APPWINDOW;
	RECT fullscreenRect = destinationPosition;
	AdjustWindowRect( &fullscreenRect, style, FALSE);

	if( IsWindowVisible( *g_windowHandle ) )
	{
		style |= WS_VISIBLE;
	}

	SetWindowLong( *g_windowHandle, GWL_STYLE, style );
	SetWindowLong( *g_windowHandle, GWL_EXSTYLE, exStyle );

	SetWindowPos( *g_windowHandle, HWND_TOPMOST, fullscreenRect.left, fullscreenRect.top, ( fullscreenRect.right - fullscreenRect.left ), ( fullscreenRect.bottom - fullscreenRect.top ), ( SWP_NOACTIVATE | SWP_FRAMECHANGED ) );
...
int WINAPI WinMain( HINSTANCE applicationInstanceHandle, HINSTANCE, LPSTR commandLineString, int )
{
	...
	CreateOpenGLWindow( applicationInstanceHandle, CLIENT_ASPECT, INCURSION_2D_APP_NAME, WindowsMessageHandlingProcedure );
	TheApp* g_theApp = new TheApp();

	while( !g_forceQuit && !g_theApp->IsQuitting() )		// Program main loop; keep running frames until it's time to quit
	{
		g_theApp->RunFrame();
	}
